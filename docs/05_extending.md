...
Neither list below is exhaustive — some message types need extra plumbing (e.g. `GroupUpdate` also
touches the group registry in
...
3. **Write the parsed message class** with a `from_message_envelope(...)` classmethod, following an
   ...
   for one that reaches back into the envelope's nested data. Subclass `BaseMessage` or
   `BaseMessageWithGroup` from
...
4. **Wire it into the parser.** Add a branch in `_parse_main_messages` and/or `_parse_sync_messages` in
   [`parser.py`](https://github.com/signalbot-org/signalbot/blob/main/src/signalbot/messages/parser.py),
   and extend the `ReceivedMessage` type alias.
...
5. **Write a `Context` class** in
   ...
   (pattern: `TypingHandler`) with one abstract `handle_xxx(self, context: ...)` method.
...
7. **(Optional) Write a trigger decorator** in
   ...
   handlers for this type commonly filter on a field — follow `reaction_triggered` as the smallest
...
8. **Write a test.** Follow
   ...
   `parse(signal, raw_json_str)`, and assert `isinstance(result, YourClass)` plus field values. Add
   dispatch coverage in
...
9. **Write an example handler** under
   ...
   (`@text_triggered` + `context.react(...)`) or
   [`examples/handlers/reaction.py`](https://github.com/signalbot-org/signalbot/blob/main/examples/handlers/reaction.py)
   (`ReactionHandler` + `@reaction_triggered`) as templates, and register it in one of the example bots
...
4. **Expose it on `SignalAPI`** if it's a new section
...
5. **Add a method on the matching `*Actions` class** in
   ...
   (pattern: `MessageActions.remote_delete` in
   ...
   useful (e.g. `SentMessage`).

6. **Wire it up if it's a new Actions class** — instantiate and attach it to `SignalBot` in
...
7. **Add a `Context` convenience method** in
   ...
   if handlers should be able to call it directly — follow how `context.react(...)` /
